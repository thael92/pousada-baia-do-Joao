...
export const Rooms: React.FC<RoomsProps> = ({ rooms, filters, onClearFilters, isSearchingAPI }) => {
    const { t } = useLanguage();
    const [selectedRoomDetails, setSelectedRoomDetails] = useState<RoomItem | null>(null);
    const [selectedRoomBooking, setSelectedRoomBooking] = useState<{ room: RoomItem, checkIn: string, checkOut: string, guests: number } | null>(null);

    const handleOpenBooking = (checkIn: string, checkOut: string, guests: number) => {
        if (selectedRoomDetails) {
            setSelectedRoomBooking({ room: selectedRoomDetails, checkIn, checkOut, guests });
    ...
    const filteredRooms = filters.isActive
        ? rooms.filter(room => {
            ...
            const matchesAvailability = room.isAvailable !== false;
            ...
            return matchesCapacity && matchesPrice && matchesAvailability && matchesFeatures;
        })
    ...
    return (
        <section id="rooms" className="py-20 bg-pousada-sand">
            <div className="container mx-auto px-6">
                    ...
                    <div className="flex flex-col gap-6 mb-12">
                        <div className="flex flex-col md:flex-row justify-center items-center gap-8 text-gray-600 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 max-w-5xl mx-auto w-full">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <span className="block text-2xl font-bold text-pousada-green">{filteredRooms.length} {t.rooms.total}</span>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <span className="block text-2xl font-bold text-pousada-green">
                                        {filteredRooms.reduce((acc, r) => acc + r.capacity, 0)} {t.booking.guests}
                                    </span>
                        ...
                        {filters.isActive && !isSearchingAPI && (
                ...
                {isSearchingAPI ? (
                    <div className="text-center py-24 bg-white rounded-[3rem] shadow-sm border border-gray-100 max-w-4xl mx-auto animate-pulse">
                        <div className="w-16 h-16 border-4 border-pousada-green border-t-transparent rounded-full animate-spin mx-auto mb-6"></div>
                        <h3 className="font-serif text-2xl font-bold text-gray-800 mb-2">Buscando disponibilidade...</h3>
                        <p className="text-gray-500 text-lg mb-8">Consultando nossas tarifas na data desejada.</p>
                    </div>
                ) : filteredRooms.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-8">
                        {filteredRooms.map((room, index) => (
                            <Reveal key={room.id} delay={index * 50}>
                                <RoomCard room={room} onSelect={setSelectedRoomDetails} />
                            </Reveal>
                        ))}
                    </div>
                ) : (
                ...
                {selectedRoomBooking && (
                    <BookingModal
                        room={selectedRoomBooking.room}
                        isOpen={!!selectedRoomBooking}
                        onClose={() => setSelectedRoomBooking(null)}
                        initialCheckIn={selectedRoomBooking.checkIn || (filters.isActive ? filters.checkIn : '')}
                        initialCheckOut={selectedRoomBooking.checkOut || (filters.isActive ? filters.checkOut : '')}
                        initialGuests={selectedRoomBooking.guests || (filters.isActive ? filters.guests : 2)}
                    />
                )}
            </div>
        </section>
